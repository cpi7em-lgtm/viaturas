...
- POST /api/auth/sync-convex -> sincroniza usuário com Convex (cria como viewer se não existir)
"""
...
# ===== JWT =====
JWT_SECRET = os.environ.get("JWT_SECRET", "viaturas-pmesp-cpi7-2026-secret")
JWT_TTL = int(os.environ.get("JWT_TTL", "604800"))  # FIX (William 2026-07-10): 7 dias (era 24h, William sofria com expiracao recorrente)

# ===== Convex =====
CONVEX_URL = os.environ.get("CONVEX_URL", "http://convex-backend-viaturas:3210")
...
# FIX (William 2026-08-17): ADMIN_EMAILS estava referenciado sem ser definido
# Causava NameError em login de qualquer PM nao-admin (ex: Peres gestor)
# Solucao: variavel vazia por padrao (pode ser setada via env ADMIN_EMAILS=...)
...
app = FastAPI(title="Viaturas CPI-7 Auth", version="1.0.0")
...
@app.get("/api/health")
def health():
    return {"ok": True, "service": "viaturas-pmesp-auth", "ts": int(time.time())}
...
async def login_soap(body: LoginBody):
    """Login via SOAP CPD PM. Cria usuário viewer no Convex Viaturas se primeiro acesso."""
    ...
    if not pm_data:
        return {"ok": False, "erro": "Não foi possível obter dados do PM"}

    # 3) Cria/atualiza usuário no Convex
    ...
    if cpf in ADMIN_CPFS or (pm_data.get("email", "").lower() in ADMIN_EMAILS):
        role = "admin"
        print(f"[viaturas-auth] ADMIN detectado: cpf={cpf} email={pm_data.get('email')}")
    else:
        role = "viewer"
    ...
    print(f"[viaturas-auth] sync_res: {sync_res}")
    # Extrai userId, unitId, isMaster, viaturasRole, unidades
    convex_user_id = None
    convex_unit_id = None
    convex_is_master = None
    convex_viaturas_role = None
    convex_unidades_gestor = None
    convex_unidades_editor = None
    convex_escopo = None
    if isinstance(sync_res, dict):
        if "value" in sync_res and isinstance(sync_res["value"], dict):
            ...
            # FIX (William 2026-08-21): extrai TUDO do user do Convex
            # pra montar o pm_data completo do JWT
            user_obj = sync_res["value"].get("user")
            if user_obj:
                convex_is_master = user_obj.get("isMaster")
                convex_viaturas_role = user_obj.get("viaturasRole")
                convex_unidades_gestor = user_obj.get("unidadesGestor")
                convex_unidades_editor = user_obj.get("unidadesEditor")
                convex_escopo = user_obj.get("escopo")
        elif "userId" in sync_res:
            convex_user_id = sync_res["userId"]
            if isinstance(sync_res, dict):
                convex_unit_id = sync_res.get("unitId")
    print(f"[viaturas-auth] convex_user_id: {convex_user_id} unit: {convex_unit_id} isMaster: {convex_is_master} viaturasRole: {convex_viaturas_role}")
    ...
    print(f"[viaturas-auth] final_user_id: {final_user_id} (convex_user_id={convex_user_id!r}, email_key={email_key!r})")
    ...
    # FIX (William 2026-08-21): isMaster/viaturasRole/unidades/escopo do Convex
    # Esses campos nao vem do SOAP, vem do createOrUpdatePMUser.
    # Sem isso, o frontend nao tem como saber o role do user no app.
    if convex_is_master is not None:
        pm_data["isMaster"] = convex_is_master
    if convex_viaturas_role is not None:
        pm_data["viaturasRole"] = convex_viaturas_role
    if convex_unidades_gestor is not None:
        pm_data["unidadesGestor"] = convex_unidades_gestor
    if convex_unidades_editor is not None:
        pm_data["unidadesEditor"] = convex_unidades_editor
    if convex_escopo is not None:
        pm_data["escopo"] = convex_escopo
    ...
    payload = {
        ...
        "iat": now,
        "exp": now + JWT_TTL,
        "aud": "viaturas",  # audience: separa do Materiais (aud="materiais")
        "app": "viaturas",  # app: redundante com aud, facilita filtro
        "pm": pm_data,
    }
...
# ============================================================
# SAT (Sistema de Acessos a Talentos) - proxy pra intranet PM
# FIX (William 2026-08-21): re-adicionado endpoint /api/sat/consulta
# que tinha sido perdido em deploys anteriores
#
# IMPORTANTE (William 2026-08-21, da skill sat-pmesp):
# - URL: HTTP (NAO HTTPS) + IP direto 10.61.9.19
# - HTTPS do sistemasadmin.intranet.policiamiltar.sp.gov.br ta MORTO desde 2026-08-11
# - DNS nao resolve sistemasadmin no container, IP direto eh mais confiavel
# - Encoding: iso-8859-1
# ============================================================
SAT_URL = "http://10.61.9.19/sat/consultaReply.asp"

def _sat_consultar(re_num: str) -> dict:
    """Consulta o SAT da PMESP por RE. Retorna dict com dados do PM ou erro."""
    import re as _re
    import urllib.request as _ur
    re_num = (re_num or "").strip()
    ...
    if not m_bloco:
        return {"encontrado": False, "erro": "Bloco OPM nao encontrado"}
    pm_opm = m_bloco.group(1).strip()
    # Remove code SIAFEM
    sem_code = _re.sub(r"\s*\(\d{9}\)\s*", " ", pm_opm).strip()
    # Posto: aceita numerado "1. TENENTE PM", "2. SGT PM" etc
    m_posto = _re.match(r"^([0-9]*\.?\s*[A-Z][A-Z\s/]+?PM)\s+(.+)$", sem_code)
    posto = ""
    nome = sem_code
    opm = ""
    if m_posto:
        ...
        # OPM eh o ULTIMO token
        m_opm = _re.search(r"\s+([A-Z][A-Z0-9/\-]+)\s*$", resto)
        if m_opm:
            opm = m_opm.group(1).strip()
            nome = resto[:m_opm.start()].strip()
        else:
            nome = resto
    m_code = _re.search(r"\((\d{9})\)", texto)
    opm_code = m_code.group(1) if m_code else ""

    # FIX (William 2026-08-24): Habilitacao pode ter VARIAS linhas (categorias
    # diferentes - ex: PM tem A, B, D). Pega TODAS e retorna a CATEGORIA
    # MAIS ALTA e MAIS RECENTE (a que vale hoje).
    # Ordem das categorias (maior -> menor): E > D > C > B > A
    RANK_CAT = {"E": 5, "D": 4, "C": 3, "B": 2, "A": 1}
    cnh_cat = ""
    boletim = ""
    data_prova = ""
    cassada = False
    publicacoes: list = []  # FIX (William 2026-08-24): todas as habilitacoes
    # Pega o bloco de habilitacao: tudo entre "Cassada" e fim (ou "ATEN")
    m_hab = _re.search(r"Cassada\s+(.+?)(?=\s*ATEN|\Z)", texto, _re.DOTALL)
    if m_hab:
        bloco = m_hab.group(1).strip()
        # Quebra em linhas (cada habilitacao eh uma "linha" separada por 2+ espacos)
        # Formato de cada item: "A OF.Nº-CPI7-034/6014 20/04/2014 Não"
        itens = _re.findall(
            r"([A-E])\s+(\S+(?:\s+\S+)*?)\s+(\d{2}/\d{2}/\d{4})\s+(Sim|N\u00e3o|N\u00c3o)",
            bloco
        )
        if itens:
            # FIX (William 2026-08-24): monta lista de publicacoes (uma por categoria)
            publicacoes = [
                {
                    "categoria": cat,
                    "boletim": bol.strip(),
                    "data": dt,
                    "cassada": cass.lower().startswith("s"),
                }
                for (cat, bol, dt, cass) in itens
            ]
            # Escolhe a categoria mais alta; em empate, a data mais recente
            def parse_data(s):
                try:
                    d, m, y = s.split("/")
            ...
            melhor = None
            for (cat, bol, dt, cass) in itens:
                score = (RANK_CAT.get(cat, 0), parse_data(dt))
                if melhor is None or score > melhor[0]:
                    melhor = (score, cat, bol.strip(), dt, cass.lower().startswith("s"))
            if melhor:
                _, cnh_cat, boletim, data_prova, cassada = melhor
    return {
        "encontrado": True,
        "re": re_num,
        ...
        "opm": opm,
        "opmCode": opm_code,
        "cnhCategoria": cnh_cat,
        "boletim": boletim,
        "dataProva": data_prova,
        "cassada": cassada,
        # FIX (William 2026-08-24): lista TODAS as publicacoes de habilitacao
        # (PMs podem ter varias - A, B, C, D, E). cnhCategoria/boletim acima
        # eh a de MAIOR rank (D > C > B > A) e MAIS RECENTE
...
async def sat_consulta(re: str = ""):
    """Consulta SAT por RE. Retorna dados do PM ou erro."""
    return _sat_consultar(re)
...
async def promote(body: dict, authorization: Optional[str] = Header(None)):
    """Promove usuário (somente admin pode chamar)."""
    token = bearer(authorization)
    if not token:
        raise HTTPException(401, "Token ausente")
    payload = verify_token(token)
