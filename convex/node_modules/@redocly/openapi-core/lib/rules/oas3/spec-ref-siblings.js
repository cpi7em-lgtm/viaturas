export const SpecRefSiblings = () => {
    return {
        ref: {
            leave(ref, { report, location, type, specVersion }) {
                const allowsRefSiblings = specVersion !== 'oas3_0';
                if ((allowsRefSiblings && type.name === 'Schema') || type.name === 'PathItem')
                    return;
                for (const key of Object.keys(ref)) {
                    if (key === '$ref')
                        continue;
                    if (allowsRefSiblings && (key === 'summary' || key === 'description'))
                        continue;
                    report({
...
//# sourceMappingURL=spec-ref-siblings.js.map