...
export async function resolveConfig({ rawConfigDocument, configPath, externalRefResolver, customExtends, skipPluginEval, }) {
    ...
    if (isBrowser) {
        ...
        const plugins = await resolvePlugins(pluginsOrPaths.map((p) => (isPluginResolveInfo(p) ? p.absolutePath : p)), rootConfigDir, skipPluginEval);
        resolvedPlugins = [...plugins, defaultPlugin];
    }
    const bundledConfig = bundleConfig(rootDocument, deepCloneMapWithJSON(resolvedRefMap), resolvedPlugins, skipPluginEval);
...
export async function resolvePlugins(plugins, configDir, skipPluginEval = false) {
    if (!plugins)
        return [];
        ...
        return (await Promise.all(pluginInstances.map(async (pluginInstance) => {
            ...
            if (pluginInstance.rules) {
                if (!pluginInstance.rules.oas3 &&
                    ...
                    throw new Error(`Plugin rules must have \`oas3\`, \`oas2\`, \`async2\`, \`async3\`, \`arazzo\`, \`overlay1\`, \`openrpc1\`, or \`graphql\` rules "${p}.`);
                }
                plugin.rules = {};
                if (pluginInstance.rules.oas3) {
