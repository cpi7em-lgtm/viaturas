...
import { isAbsoluteUrl, replaceRef, isExternalValue, isRef, parseRef, pointerBaseName, refBaseName, isMappingRef, } from '../ref-utils.js';
...
export function makeBundleVisitor({ version, dereference, rootDocument, resolvedRefMap, keepUrlRefs, componentRenamingConflicts = 'warn', componentNamesStrategy = 'basename', }) {
    let components;
    let rootLocation;
    // a composed $ref in the chain is the effective target, so the composition survives bundling
    const effectiveTarget = (resolved) => resolved.chain?.[0] ?? { node: resolved.node, location: resolved.location };
    const firstSchemaLocationByName = new Map();
    ...
    const visitor = {
        ref: {
            leave(node, ctx, resolved) {
                ...
                if (target.location.source === rootDocument.source &&
    ...
    if (version === 'oas3') {
        visitor.Discriminator = {
            leave(discriminator, ctx) {
                ...
                discriminator.defaultMapping = saveComponent(schemaComponentType, effectiveTarget(resolved), ctx);
            },
    ...
    function componentNameFromTitle(target, componentsGroup, ctx) {
        const { node } = target;
        ...
        if (componentsGroup[key] && !isEqualOrEqualRef(componentsGroup[key], target, ctx)) {
            return {
                key,
                problem: {
                    message: `Title "${title}" maps to component name \`${key}\`, ` +
    ...
    function componentNameFromBasename(target, componentsGroup, ctx) {
        const prevName = pointerBaseName(target.location.pointer) || refBaseName(target.location.source.absoluteRef);
        let name = prevName;
        for (let serialId = 2; componentsGroup[name] && !isEqualOrEqualRef(componentsGroup[name], target, ctx); serialId++) {
