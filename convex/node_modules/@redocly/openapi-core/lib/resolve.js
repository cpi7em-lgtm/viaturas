...
import { isRef, joinPointer, escapePointerFragment, parseRef, isAbsoluteUrl, isAnchor, isExternalValue, isRefWithSiblings, Location, } from './ref-utils.js';
    ...
    function resolveRefsInParallel(rootNode, rootNodeDocument, rootNodePointer, type) {
                ...
                const promise = followRef(rootNodeDocument, node, {
                    ...
                    if (resolvedRef.resolved) {
                        ...
                        for (const chainHop of resolvedRef.chain ?? []) {
                            resolveRefsInParallel(chainHop.node, chainHop.document, chainHop.location.pointer, type);
                        }
                    }
                });
        ...
        async function followRef(document, ref, refStack) {
            ...
            if (resolvedRef.document && isRef(target)) {
                ...
                const chainHop = isRefWithSiblings(target)
                    ? {
                        node: target,
