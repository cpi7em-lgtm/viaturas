...
function readManifestVersion() {
    try {
        const pathToManifest = path.join(path.dirname(fileURLToPath(import.meta.url)), "..", "..", "package.json");
 ...
 * @param delimiter
 */
export function trimMargin(delimiter = "|") {
...
cli
    ...
    .option("-m, --displayMode <mode>", "Either display the result of the analysis as a 'webapp', 'graph', 'file-tree' or 'raw'", "webapp")
    ...
    .option("-it, --trackTypeOnlyDependencies", "Enable dependency tracking for TypeScript 'import type' statements", true)
    .option("-nit, --no-trackTypeOnlyDependencies", "Disable dependency tracking for TypeScript 'import type' statements")
    .option("-ig, --ignorePattern <pattern>", "Provide ignore pattern to exclude files from the analysis", collectIgnorePatterns, [])
    ...
    .option("-uf, --showUnusedFiles", "Search for unused files in the graph", false)
    ...
    .option("-fw, --watch", "Watch for changes depending on the provided '--cwd' or defaults to process.cwd() and re-run the analysis when a supported file is added/deleted/modified", false)
    ...
    .usage(trimMargin("|")(`
        ...
        `))
    .action((name, commandAndOptions) => runTerminalApplicationFromCLI(name, commandAndOptions));
