...
export const watchModeStatus = {
    watching_for_changes: "Watching for graph changes in current working directory...",
    changes_detected: "Changes detected"
};
function toDotlessExtension(fileExtension) {
...
async function getRootGitIgnorePath(cwd) {
    try {
...
async function retrieveGitIgnoredEntries(cwd) {
    const gitIgnore = await getRootGitIgnorePath(cwd);
    return pipe(gitIgnore, Option.map((gitIgnorePath) => fs
        .readFile(gitIgnorePath)
        .then((c) => gitignoreParse(c).patterns)
        .catch(() => [])), Option.getOrElse(() => []));
}
...
export async function registerWatchMode({ cwd, ignorePatterns, fileExtensions, verbose, onChangesDetected }) {
    /**
    ...
    return watcher.subscribe(cwd, (_err, _events) => {
        if (verbose) {
            changesCount++;
            if (changesCount === 1) {
                logger.stdout("\n");
