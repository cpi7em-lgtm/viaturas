...
import { displayCircularDependencies, displayDependenciesReport } from "./ui/console/dependencies.js";
...
export function makeSkottRunner(config) {
    let isFirstRun = true;
    ...
    if (config.incremental) {
        console.log(`\n ${kleur
            .bold()
            .yellow("`incremental` mode is experimental. Please report any issues you encounter.")}`);
    }
    return async () => {
        ...
        try {
            ...
            isFirstRun = false;
            const skottResult = await runFromRuntimeConfig(config);
            ...
            if (error.message) {
                ...
                console.error(`\n ${kleur
                    .bold()
                    .red("Unexpected error. Please use `--verbose` flag and report" +
...
function displayInitialGetStructureTime(files, startTime) {
...
export async function runTerminal(run, runtimeConfig, terminalOptions) {
    ...
    if (terminalOptions.displayMode === "file-tree") {
        const fileTreeComponent = new CliComponent(() => renderFileTree(skottInstance, {
            circularMaxDepth: runtimeConfig.circularMaxDepth,
            exitCodeOnCircularDependencies: terminalOptions.exitCodeOnCircularDependencies,
        ...
        const graphComponent = new CliComponent(() => renderGraph(skottInstance, {
            circularMaxDepth: runtimeConfig.circularMaxDepth,
            exitCodeOnCircularDependencies: terminalOptions.exitCodeOnCircularDependencies,
            showCircularDependencies: terminalOptions.showCircularDependencies
        }));
        ...
        const circularDepsComponent = new CliComponent(() => displayCircularDependencies(skottInstance, {
            circularMaxDepth: runtimeConfig.circularMaxDepth,
            exitCodeOnCircularDependencies: terminalOptions.exitCodeOnCircularDependencies,
            /**
    ...
    if (terminalOptions.displayMode !== "webapp") {
        await new Promise((resolve) => {
            const depsReportComponent = new CliComponent(() => displayDependenciesReport(skottInstance, {
