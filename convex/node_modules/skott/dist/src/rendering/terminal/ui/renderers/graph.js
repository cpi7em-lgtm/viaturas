...
function render(graph, filesInvolvedInCycles, nodesWithBodyBindings) {
    const leftArrow = `${kLeftSeparator}>`;
    for (const [nodeId, nodeValue] of Object.entries(graph)) {
        const localStore = nodeValue.adjacentTo.reduce((store, current) => {
        ...
        if (filesInvolvedInCycles.includes(nodeId)) {
            console.log(`${makeIndents(1)} ${kleur.red().underline().bold(nodeId)} ${kleur
                .bold()
                .yellow("♻️")}`);
        }
        else {
            console.log(`${makeIndents(1)} ${kleur
                .blue()
                .underline()
                .bold(nodeId)} (self=${parentNodeSize}, imported=${totalOfChildrenSize})`);
        }
        for (const subNode of nodeValue.adjacentTo) {
            ...
            if (filesInvolvedInCycles.includes(subNode)) {
                ...
                console.log(`${makeIndents(3)} ${kleur.bold().yellow(leftArrow)} ${kleur
                    .bold()
...
export function renderGraph(skottInstance, options) {
    ...
    const nodesWithBodyBindings = new Map();
    for (const [nodeId, nodeValue] of Object.entries(graph)) {
