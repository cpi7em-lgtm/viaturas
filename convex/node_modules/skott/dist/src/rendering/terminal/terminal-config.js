...
const terminalSchema = D.struct({
    watch: D.boolean,
    displayMode: D.union(D.literal("raw"), D.literal("file-tree"), D.literal("graph"), D.literal("webapp"), 
    /**
     ...
     */
    D.literal("md"), D.literal("json"), D.literal("svg"), D.literal("png")),
    showCircularDependencies: D.boolean,
    showUnusedDependencies: D.boolean,
    exitCodeOnCircularDependencies: D.number
});
export function ensureNoIllegalTerminalConfig(terminalConfig, apiConfig) {
    const result = terminalSchema.decode(terminalConfig);
    if (result._tag === "Left") {
        return Either.left(`Invalid terminal configuration: ${D.draw(result.left)}`);
    }
    if (terminalConfig.showUnusedDependencies &&
        !apiConfig.trackThirdPartyDependencies) {
        return Either.left("`--trackThirdPartyDependencies` must be provided when searching for unused dependencies.");
    }
    /**
    ...
    if (terminalConfig.displayMode === "webapp") {
        if (terminalConfig.showCircularDependencies) {
            return Either.left("`--showCircularDependencies` can't be used when using `--displayMode=webapp`");
        }
        if (terminalConfig.showUnusedDependencies) {
            return Either.left("`--showUnusedDependencies` can't be used when using `--displayMode=webapp`");
        }
        if (terminalConfig.showUnusedFiles) {
            return Either.left("`--showUnusedFiles` can't be used when using `--displayMode=webapp`");
        }
    }
    if (apiConfig.entrypoint && terminalConfig.showUnusedFiles) {
        return Either.left("`--showUnusedFiles` can't be used when using providing an entrypoint.");
    }
    if (terminalConfig.watch) {
        if (terminalConfig.displayMode !== "webapp" &&
            ...
            return Either.left("`--watch` needs either `raw`, `file-tree`, `graph` or `webapp` display mode");
        }
    }
