 ...
 */
function isEcmaScriptModuleExport(estreeNode) {
    /*
...
function isEcmaScriptModuleImport(estreeNode) {
    return estreeNode.type === "ImportDeclaration";
}
function isEcmaScriptModuleDeclaration(estreeNode) {
    return (isEcmaScriptModuleImport(estreeNode) || isEcmaScriptModuleExport(estreeNode));
}
function isTypeScriptImportType(node) {
    const hasOnlyTypeSpecifiers = node.specifiers &&
        node.specifiers.every((_) => _.importKind === "type");
    return node.importKind === "type" || hasOnlyTypeSpecifiers;
}
export function extractModuleDeclarations(node, moduleDeclarations, trackTypeOnlyDependencies = true) {
