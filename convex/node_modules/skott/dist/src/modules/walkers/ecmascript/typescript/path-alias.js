...
function resolveAliasToRelativePath(moduleDeclaration, baseAlias, baseAliasDirname, logger) {
    /**
     * When having a path alias like "foo/*": ["core/lib/*"], we want to map
     * any segment such as "foo/lib/baz" to "core/lib/baz".
     */
    try {
        logger.info(`Extracting "${moduleDeclaration}" base path using path alias base directory "${baseAliasDirname}"`);
        const baseDirWithSlash = baseAliasDirname.concat("/");
        const startIndex = moduleDeclaration.indexOf(baseDirWithSlash) + baseDirWithSlash.length;
        const modulePathWithoutAliasBaseDirname = moduleDeclaration.substring(startIndex);
        logger.info(`Attempt to map alias path to real file-system path.` +
            ` Join "${baseAlias}" -> "${modulePathWithoutAliasBaseDirname}"`);
        const realPathWithAlias = path.join(baseAlias, modulePathWithoutAliasBaseDirname);
        return Option.some(realPathWithAlias);
    }
    catch {
        logger.failure(`Could not resolve ${moduleDeclaration} alias path to real file-system path` +
            ` using ${baseAlias} and ${baseAliasDirname}.`);
        return Option.none();
    }
}
function isRootAliasSymbol(segment) {
    return !segment.includes("/");
}
async function readTSConfig(tsConfigPath, logger, fileReader) {
    let tsconfig = "";
    try {
        const localTSConfigPath = path.join(fileReader.getCurrentWorkingDir(), tsConfigPath);
        tsconfig = await fileReader.read(localTSConfigPath);
        logger.success(`Successfully found tsconfig: ${localTSConfigPath}`);
    }
    catch (exception) {
        logger.failure(`An exception occured reading from tsconfig: ${tsConfigPath}. Reason ${exception.message}}`);
        /**
        ...
        try {
            ...
            logger.success(`Successfully found tsconfig at: ${thirdPartyTSConfigPath}`);
        }
        catch (exception) {
            logger.failure(`An exception occured reading from tsconfig: ${tsConfigPath}. Reason ${exception.message}}`);
        }
...
export async function buildPathAliases(fileReader, tsConfigPath, aliasLinks, logger) {
    try {
        ...
        if (paths) {
            ...
            for (const [alias, aliasedPath] of Object.entries(paths)) {
                ...
                const realPathAliasLocation = path.join(baseUrl, realPathWithoutWildcard);
                aliasLinks.set(aliasWithoutWildcard, realPathAliasLocation);
                logger.info(`Registering path alias without wildcards "${aliasWithoutWildcard}": ["${realPathAliasLocation}"]`);
            }
        }
        ...
        if (extendedTsConfigPath) {
            ...
            await buildPathAliases(fileReader, extendedTsConfigPath, aliasLinks, logger);
            logger.success(`Finished extracting path aliases from tsconfig. ${aliasLinks.size} aliases found.`);
        }
        return {
...
export function resolvePathAlias(moduleDeclaration, baseDir, aliasLinks, logger) {
    ...
    if (baseAlias) {
        const aliasPath = resolveAliasToRelativePath(moduleDeclaration, baseAlias, baseAliasDirname, logger);
        if (Option.isNone(aliasPath)) {
            logger.failure(`${moduleDeclaration} path alias could not be resolved using base path ${baseAlias}`);
        }
        return pipe(aliasPath, Option.map((resolvedPath) => path.join(baseDir, resolvedPath)));
    }
    logger.failure(`No match found for ${moduleDeclaration} using base ${baseAlias} path alias in the registered entries`);
    /**
    ...
    while (!isRootAliasSymbol(baseAliasDirname) && pathDepthAttempts < 10) {
        ...
        if (deepBaseAlias) {
            baseAlias = Option.getOrUndefined(resolveAliasToRelativePath(moduleDeclaration, deepBaseAlias, baseAliasDirname, logger));
            pathDepthAttempts = 0;
            break;
        }
    }
    if (!baseAlias) {
        logger.failure(`No match found for ${moduleDeclaration} and corresponding segments of ${baseAlias}` +
            ` path alias in the registered entries. Path depth attempts: ${pathDepthAttempts}`);
        return Option.none();
    }
    return Option.some(path.join(baseDir, baseAlias));
}
export function isTypeScriptRelativePathWithNoLeadingIdentifier(baseUrl, moduleDeclaration) {
    return (baseUrl !== undefined &&
        !moduleDeclaration.startsWith(".") &&
        !moduleDeclaration.startsWith("./"));
}
export function isTypeScriptPathAlias(moduleDeclaration, aliasLinks) {
